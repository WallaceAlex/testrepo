const employees = [
 ...
 // Function to display all employees, map method
function displayEmployees(){
    const totalEmployees = employees.map((employee, index) => `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.department} - $${employee.salary}</p>`).join('');
    document.getElementById('employeesDetails').innerHTML = totalEmployees;
}

// Reduce method to show total
function calculateTotalSalaries() {
    const totalSalaries = employees.reduce((acc, employee) => acc + employee.salary, 0);
    alert(`Total Salaries: $${totalSalaries}`);
}

// Filter method to show only HR
...
// Find method to locate content based on ID
...
function findEmployeeBySpec(specInput) {
    const foundSpec = employees.find(employee => employee.specialization === specInput);
